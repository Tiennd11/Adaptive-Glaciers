...
for l_mul in [4.0, 2.0, 1.0, 0.625]:
    ...
    alpha_2 = 0.1  # threshold for damage
    ...
    mesh = Mesh()
    with XDMFFile("mesh/adaptive/mesh.xdmf") as infile:
        infile.read(mesh)
    ...
    xdmf = XDMFFile("output/adaptive/"+str(l_mul)+"/output.xdmf")
    ...
    def get_crack_tip_coord(dmg, Dx,  damage_threshold=0.1):
    ...
    def problem(mesh, ms_iterations=1, u_adaptive=None, p_adaptive=None, cdf_adaptive=None):
        ...
        t = Expression(("(h - x[2] >= 0 ?-rho_H2O * grav*(h - x[2]) : 0)",
                    0, 0), h=H1, rho_H2O=rho_sea, grav=grav, degree=1)
    ...
    csv_file_path = "output/adaptive/crevasse_depth_"+str(l_mul)+".csv"
